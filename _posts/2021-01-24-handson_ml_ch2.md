...
```python
# 상관관계의 시각화
# 대각선 부분은 산점도가 직선으로 표현되기 때문에 히스토그램으로 대체 
...
```python
# 결측치 처리를 위한 3가지 방법
# 3번을 사용할 경우, 이후 test set에도 train set에서 사용한 값으로 결측치 대체
...
# random forest k-fold cross validation 결과확인
display_scores(tree_rmse_scores)
```
